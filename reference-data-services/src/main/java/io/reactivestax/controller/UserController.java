...
//import com.reactivestax.domain.User;
...
import io.reactivestax.validator.UserRegistrationRequestValidator;
...
//        }
    ...
    @ApiOperation(value = "/api/v1/users/{userId}", nickname = "/api/v1/users/{userId}")
    @ApiResponses(value = { //
            @ApiResponse(code = 400, message = "Bad Request"),
            @ApiResponse(code = 200, message = "OK"),
            @ApiResponse(code = 500, message = "Failure")})
    ...
    @SuppressWarnings("Duplicates")
    public ResponseEntity<RestApiResponse<RestApiStatus,User>> updateUser(@RequestBody @Valid final User user,@PathVariable String userId, HttpServletRequest request ) {
        log.debug("Start registerUser {} ", user);
        RequestMetadata requestMetadata = (RequestMetadata)request.getAttribute("request-metadata");
