...
public class BankingApp {
    ...
    private static void writeBankState2Database() {
    ...
    private static void transferMoneyBetweenDBAccounts() {
        logger.debug("Starting Money Transfer in DB:");

        String sourceAccountID = optionStrFromUser("Enter Source Account Id: ");

        String destinationAccountID = optionStrFromUser("Enter Destination Account Id: ");

        int money2Transfer = optionFromUser("How much money you want to transfer:");

        try {
            new TransferService().transferMoneyBetweenAccountsInDB(sourceAccountID, destinationAccountID,money2Transfer);
        } catch (ExceedingTotalBalanceException e) {

            logger.debug("Transfer amount is more than the balance in the account");
            e.printStackTrace();
        }
    ...
    private static void doMoneyWithdrawInDB() {
        logger.debug("Starting Money Withdraw in DB:");

        String accountID = optionStrFromUser(ENTER_ACCOUNT_ID);

        int money2WithDraw = optionFromUser("Enter money to Withdraw:");

        new MoneyWithdrawService().withDrawMoneyInDB(accountID,money2WithDraw);
    ...
    private static void doMoneyDepositInDB() {
        logger.debug("Staring money deposit in DB");
        ...
        new MoneyDepositService().depositMoneyInDB(accountID,money2Deposit);
    }
    ...
    private static void createNewCustomer() {
        ...
        new CreateCustomerService().createNewCustomer(
                customerType,
    ...
    private static void createNewDBAccount() {
        logger.debug("creating new DB account");

        String customerId = optionStrFromUser("Enter Customer ID ");
        int accountType = typeOfAccount();
        new CreateAccountService().createNewAccount(customerId, accountType);
    }
    ...
    private static int typeOfAccount() {
        while (true) {
            int typeOfAccount = optionFromUser("What kind of account: Debit, Savings, Credit Card [1/2/3] : ");
    ...
    public static void printBankOptions() {
        logger.debug("Menu:\n" +
                ...
                "3. Show Details of an Account\n" +
                "4. Do Money Deposit\n" +
                "5. Do Money Withdraw\n" +
                "6. Transfer Money between two accounts\n" +
                "7. Print Transactions of an account\n" +
                "8. Print All Account Details in the bank.\n" +
                "9. Quit\n" +
                "--------------------------------------------");
